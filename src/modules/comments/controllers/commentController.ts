...
export const createCommentController = (commentService: CommentService) => {
  ...
  commentController.get('/post/:post_id', validateRequest('query', getCommentsQuerySchema), async (c) => {
  ...
  commentController.put('/:comment_id', auth, validateRequest('json', updateCommentSchema), async (c) => {
    const { comment_id } = c.req.param();
    const { user_id } = c.get('user');
    const data = c.req.valid('json');
  ...
  commentController.get('/user/:user_id', validateRequest('query', getCommentsQuerySchema), async (c) => {
