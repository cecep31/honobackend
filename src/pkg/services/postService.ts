...
import { posts as postsModel, postsToTags, tags as tagsModel, users as usersModel } from '../../database/schema/schema'
...
export class PostService {
        ...
        try {
            ...
            if (tags.length > 0) {
                for (const tag of tags) {
                    await db.insert(tagsModel).values({ name: tag }).onConflictDoNothing();
                }
            }

            const getTags = await db.query.tags.findMany({ where: inArray(tagsModel.name, tags) });
    ...
    static async getYourPosts(user_id: string, limit = 100, offset = 0) {
        ...
        const total = await db.select({ count: count() }).from(postsModel).where(eq(postsModel.created_by, user_id));
        return { data: postsdata, total: total[0].count }
    }

    static async getPostsByTag($tag: string) {
        const tag = await db.query.tags.findFirst({ where: eq(tagsModel.name, $tag) })
        if (!tag) {
    ...
    static async getPostsRandom() {
        const postsData = await db.select().from(postsModel).orderBy(sql.raw("RANDOM()")).limit(6)
        return postsData
    }

    static async getPost(id_post: string) {
        const post = await db.query.posts.findFirst({ where: eq(postsModel.id, id_post) })
    ...
    static async deletePost(postId: string) {
        const deletedPost = await db.delete(postsModel).where(eq(postsModel.id, postId)).returning();
        if (!deletedPost) {
            throw new HTTPException(404, { message: "Post not found" });
        }
        return deletedPost;
