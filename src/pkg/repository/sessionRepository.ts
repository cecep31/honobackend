...
export class SessionRepository {
    async insertSession(data: SessionCreate) {
        const session = await db.insert(sessionModel).values(data).returning({ id: sessionModel.id, refresh_token: sessionModel.refresh_token })
        return session[0]
    }

    async getSessionByRefreshToken(refresh_token: string) {
        const session = await db.select().from(sessionModel).where(eq(sessionModel.refresh_token, refresh_token))
        return session[0]
    }
    ...
    async getSessionByUserId(user_id: string) {
        return await db.select().from(sessionModel).where(eq(sessionModel.user_id, user_id))
    }
}
